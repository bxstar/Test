...
namespace Common.DB
{
    public class DataUtil
    {
        ...
        public static string BuildInsertSQL(DataRow r, out SQLiteParameter[] ps)
        {
            DataTable t = r.Table;
            string s = "\ninsert into " + t.TableName + " ({0}) values ({1})";
            StringBuilder b1 = new StringBuilder();
            ...
            for (int i = 0; i < t.Columns.Count; i++)
            {
                if (r[i] != DBNull.Value)
                    if (b1.Length > 0)
                    {
                        ps[count] = new SQLiteParameter("@" + t.Columns[i].ColumnName, t.Columns[i].GetType());
                        ps[count].Value = r[i];
                        count++;
                    }
            }
        ...
        public static string BuildUpdateSQL(DataRow r, out SQLiteParameter[] ps)
        {
            DataTable t = r.Table;
            string s = "\nupdate " + t.TableName + " set {0} where {1}";
            StringBuilder b1 = new StringBuilder();
        ...
        public static string BuildDeleteSQL(DataRow r, out SQLiteParameter[] ps)
        {
            DataTable t = r.Table;
            ps = new SQLiteParameter[1];
            ps[0] = new SQLiteParameter("@" + t.Columns[0].ColumnName, t.Columns[0].GetType());
            ps[0].Value = r[0];
            string s = "\ndelete " + t.TableName + " where {0}";
        ...
        public static int QueryCount(string tName, string query,string type)
        {
            if (query != "")
                query = " where " + query;
            return (int)DataBase.ExecuteObject(string.Format("select count(1) from {0}{1}", tName, query),type);
        }
